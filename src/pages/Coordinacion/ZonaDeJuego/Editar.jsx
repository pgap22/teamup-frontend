...
import { editarUnaZonaDeJuego, obtenerDeportes, obtenerUnaZonaDeJuego } from "src/api";
...
export default function Editar() {
    ...
    const {isLoading, zonaDeJuego} = useFetchId(id, obtenerUnaZonaDeJuego, "zonaDeJuego");


    const { deportes } = useFetch("deportes", obtenerDeportes, deportesSelect)

    const { handleSubmit, register, setValue, registroExitoso }  = useFormulario(editarUnaZonaDeJuego);
    ...
    return (
        ...
        >
            <CoordinacionForm handleSubmit={handleSubmit} titulo={"Datos Generales"} imagenUrl={"/basquet.jpg"}>
                ...
                <ImageSelector setValue={setValue} valueLabel={"imagen_eliminadas"} imagenes={zonaDeJuego.imagenes} label={"Editar Imagenes"} />
