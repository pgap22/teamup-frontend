...
const ImageUI = ({ label, id = "image", setValue, name }) => {
  ...
  return (
      <div className="flex flex-col gap-1 w-full">
        ...
        <label onDrop={dropImagen} onDragOver={dragOver} className="border-[3px] cursor-pointer hover:bg-gray-100 transition-all py-20 border-gray-300 rounded-lg border-dashed" htmlFor={id}>
          <p className='text-gray-400 mx-auto max-w-[200px] text-center'>Haz click aqui o arrastra tus imagenes aqui !</p>
        </label>
  ...
  return (
    <div className='mt-4 grid grid-cols-[1fr_max-content]'>
      ...
      <div className='grid gap-2 grid-cols-[6ch_max-content] items-center relative'>
        <p className='text-sm font-bold border-2 rounded-sm text-center p-0.5'>{convertirTamaño(img.size)}</p>
        <MdOutlineCancel onClick={eliminarImagen} className='cursor-pointer' size={22} />
      </div>
    </div>
  );
};
